...
class ReportHTMLGenerator:
	...
	def generateErrorList(self, test):
		...
		if len(test.getWarnings()) + len(test.getWarnings()) == 0:
	...
	def generate(self):
		...
		for test_name in self.mate.getTestNameList():
			for test in self.mate.getTestsByName(test_name):
				csv += "%s,%s,%f,%f\n" % (
				test_name, (test.getMapper().getName() + " " + test.getMapper().param_string.replace(",", ";")).strip(),
				(100.0 * test.getRunResults().correct) / float(test.getRunResults().total),  test.getRunResults().total / float(test.getRunResults().maptime))
